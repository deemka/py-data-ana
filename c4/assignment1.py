...
def date_sorter():
    ...
    # Match all M/D/Y
    r1i = df.copy()
    r1 = r1i.str.extractall(r'\b0?(?P<month>\d{1,2})[-/](?P<day>\d{1,2})[-/](?P<year>(?:\d{4}\b|\d{2}\b))')[['day', 'month', 'year']]
    ...
    r2 = r2i.str.extractall(r'(?P<month>\d{1,2})[/](?P<year>(?:\d{4}\b|\d{2}\b))')[['month', 'year']]
    ...
    r3 = r3i.str.extractall(r'(?P<day>\d{1,2}) +(?P<month>' + re_month + ') +(?P<year>(?:\d{4}|\d{2}))')[['day', 'month', 'year']]
    r3.reset_index(inplace=True)

    r4i = r3i[~r3i.index.isin(list(r3.level_0))]
    r4 = r4i.str.extractall(r'(?P<month>' + re_month + ')\.? (?P<day>\d{1,2}),? +(?P<year>(?:\d{4}|\d{2}))')[['day', 'month', 'year']]
    r4.reset_index(inplace=True)

    r5i = r4i[~r4i.index.isin(list(r4.level_0))]
    r5 = r5i.str.extractall(r'\w?(?P<month>' + re_month + '),? +(?P<year>(?:\d{4}|\d{2}))')[['month', 'year']]
    r5.reset_index(inplace=True)
    r5['day'] = 1

    r6i = r5i[~r5i.index.isin(list(r5.level_0))]
    r6 = pd.DataFrame(r6i.str.extractall(r'(?P<year>(?:19\d\d)|(?:20\d\d))')['year'])
    ...
    res = pd.concat([r1, r2, r3, r4, r5, r6])
    ...
    res = res.sort_values(['year', 'month', 'day'])
